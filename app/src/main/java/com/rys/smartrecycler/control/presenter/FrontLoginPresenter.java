...
import com.rys.smartrecycler.control.model.FrontLoginModel;
 ...
 * 功能描述:
 */
public class FrontLoginPresenter extends BasePresenterImpl<FrontLoginApi.View,FrontLoginApi.Model> implements FrontLoginApi.Presenter<FrontLoginApi.View>{
    private int time = 60;
    ...
    public FrontLoginPresenter(Context context) {
        super(context);
    }

    @Override
    public FrontLoginApi.Model attachModel() {
        return new FrontLoginModel(mContext);
    }
            ...
            getModel().startGetPwdInfo(phone, new RequestCallBackListener<String>() {
                ...
                public void onSuccess(String data) {
                    if(isViewAttached()){
                        ...
                        LogController.insrtOperatorLog("投递日志", phone+"用户获取验证码");
                    }
                }
    ...
    public void startGoLogin() {
        ...
        if("".equals(phone) || phone.length() != 11){
            getView().showToast(0,"请输入正确的账号");
            return;
        }
        if("".equals(phone) || pwd.length() < 4){
            getView().showToast(0,"请输入正确的验证码");
            return;
        }
        if(isModelAttached()){
