...
public class SectionedGridDivider extends RecyclerView.ItemDecoration {
    ...
    private boolean isDraw(RecyclerView parent, View itemView, int totalCount) {
        ...
        //区分是否含有headerview
        if (itemPosition + 1 < totalCount) {
            if (adapter.hasHeader() && itemPosition == 0)
                return false;
            if (parent.getAdapter() instanceof SectionedRecyclerViewAdapter) {
                if (adapter.isSectionHeaderPosition(adapter.hasHeader() ? itemPosition - 1 : itemPosition)) {
                    return true;
                }
            }
        }
    ...
    private void drawHorizontal(Canvas c, RecyclerView parent) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (isDraw(parent, child, totalCount)) {
                ...
                int top = child.getTop() - mDividerHeight - params.topMargin;//组件在容器Y轴上的起点
