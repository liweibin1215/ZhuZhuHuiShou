...
//        window.setStatusBarColor(Color.TRANSPARENT);
     ...
     * 显示、隐藏假的透明栏/或者状态栏
     * <p>
     * 界面是白色状态栏，针对于状态栏无法改成黑色主题的版本，状态栏增加一层透明度的View
     * 但是设置activity为沉浸式模式时，需要设置颜色和透明度
     */
    public static void setTranslucentViewVisibility(Activity activity, int color, @FloatRange(from = 0.0, to = 1.0) float alpha, boolean isShow) {

        if (Build.VERSION.SDK_INT >= 21) {
            //魅族、小米、6.0以上 这些可以改变状态栏为黑色的 设置状态栏透明化
            if (isFlyme4Later() || isMIUI6Later() || Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                //21已经可以直接设置状态栏颜色，设置为透明
                activity.getWindow().setStatusBarColor(mixtureColor(DEFAULT_COLOR, DEFAULT_ALPHA));
            }else{
                //5.0 - 6.0之间不能设置黑色主题的保留状态栏颜色
                activity.getWindow().setStatusBarColor(mixtureColor(color, alpha));
            }
        } else if (Build.VERSION.SDK_INT >= 19) {//19以上才开启了沉浸式模式，但是是假的状态栏
            View translucentView = activity.getWindow().getDecorView().findViewById(android.R.id.custom);
            //魅族、小米 : 这些可以改变状态栏为黑色的 就不需要半透明层
            if (isFlyme4Later() || isMIUI6Later()){
                isShow = false;
            }else{
                //这里就根据isShow 参数实际需要进行显示隐藏
            }
            if (translucentView != null)
                translucentView.setVisibility(isShow ? View.VISIBLE : View.GONE);
        }
    }
