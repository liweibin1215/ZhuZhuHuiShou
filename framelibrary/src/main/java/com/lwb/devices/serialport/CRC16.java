...
public class CRC16 {
     ...
     * @return CRC16校验值
     */
    public static int calcCrc16L(byte[] data, int offset, int len, int preval) {
        int ucCRCHi = (preval & 0xff00) >> 8;
        int ucCRCLo = preval & 0x00ff;
        int iIndex;
        for (int i = 0; i < len; ++i) {
     ...
     * @return
     */
    public static int calcCrc16L(byte[] data) {
        return calcCrc16L(data, 0, data.length, 0xffff);
    }
    // 测试
    public static void main(String[] args) {
        // 0x02 05 00 03 FF 00 , crc16=7C 09
        //int crc = CRC16.calcCrc16(new byte[] { 0x02, 0x05, 0x00, 0x03, (byte) 0xff, 0x00 });
        int crc = CRC16.calcCrc16(new byte[] { HexToByte("01"), HexToByte("06"), HexToByte("00"), HexToByte("32"), HexToByte("00"), HexToByte("00")});
